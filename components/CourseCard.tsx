...
const CourseCard: React.FC<CourseCardProps> = ({ subject, onClick, lang }) => {
  const content: any = {
    ...
    en: { desc: 'Simplified lessons and exercises for Baccalaureate and the Moroccan curriculum', action: 'Browse Lessons' },
    fr: { desc: 'Cours simplifiés et exercices pratiques pour le Baccalauréat et le programme marocain', action: 'Voir les cours' }
  };
  ...
  return (
    ...
    >
      ...
      <div className="icon-3d w-16 h-16 rounded-2xl flex items-center justify-center text-3xl mb-6 shadow-xl" style={{ backgroundColor: subject.color + '20', color: subject.color }}>
        <div className="icon-3d-base"></div>
      ...
      <p className="text-gray-500 dark:text-gray-400 text-sm mb-6 leading-relaxed">{t.desc}</p>
