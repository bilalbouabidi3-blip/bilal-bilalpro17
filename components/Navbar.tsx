          ...
          <div className="hidden lg:flex items-center gap-6 font-bold text-gray-500 dark:text-gray-400 text-sm">
            {navItems.map((item) => (
              <button 
                ...
                className={`flex items-center gap-2 transition-all duration-200 hover:text-blue-600 py-1 relative ${currentSection === item.id ? 'text-blue-600 after:content-[""] after:absolute after:-bottom-1 after:left-0 after:w-full after:h-0.5 after:bg-blue-600' : ''} ${item.id === AppSection.AI_TOOLS ? 'group icon-3d-container' : ''}`}
              >
                {item.id === AppSection.AI_TOOLS && <i className={`fas ${item.icon} icon-3d text-blue-500`}></i>}
            ...
            <div className="flex-1 space-y-2 overflow-y-auto custom-scrollbar">
              {navItems.map((item) => (
                <button 
                  ...
                  className={`w-full flex items-center gap-4 p-4 rounded-2xl font-bold transition-all group ${currentSection === item.id ? 'bg-blue-600 text-white shadow-lg' : 'text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-slate-800'} ${item.id === AppSection.AI_TOOLS ? 'icon-3d-container' : ''}`}
                >
                  <div className={`w-8 h-8 rounded-lg flex items-center justify-center transition-all ${currentSection === item.id ? 'bg-white/20' : 'bg-blue-50 dark:bg-blue-900/30 text-blue-600'} ${item.id === AppSection.HOME ? 'group-hover:scale-110 group-hover:shadow-lg' : ''}`}>
                    <i className={`fas ${item.icon} ${item.id === AppSection.AI_TOOLS ? 'icon-3d' : ''}`}></i>
                  </div>
                  {item.label}
                </button>
              ))}
